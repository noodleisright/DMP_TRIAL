...
## 이제 orientation에 대해서
### --5) 초기 데모데이터에서 ori부분만 가져온다
### --6) ori부분에 대해서만 ProMPs 학습 , via point도 qw,qx,qy,qz에 대해서만 맞추기
### --7) 이때 rel구할때는 pose-ProMPs에 나온 그 궤적 pos 집합으로 구한다.
### --8) pos weight에 vstack으로 ori weight를 합침
### --9) 저거 까지 다학습할때 최종결과 확인--> 이 mean을 저장!
import timeit
...
if __name__ == '__main__':
    ...
    Learned_model_folder = r"C:\김창현작업\Kch_project\Pycharmproject\MAT_REP_RWR_moving_cube\3D_Moving_cube\Learned_model_parameter_3dof"
    Given_env_vector = np.load( Learned_model_folder + "/" + "inital_env_chavec[ 8.89403109 11.42291753 16.5813178 ].npy")
    ws_x_scale = 25;
    ws_y_scale = 30;
    ws_z_scale = 25;
    ...
    #env2
    # via1 = np.array([5.09 / ws_x_scale, 4.82 / ws_y_scale, -14.64 / ws_z_scale])
    # via2 = np.array([-1.483 / ws_x_scale, 0 / ws_y_scale, -0.99 / ws_z_scale])
    # via3 = np.array([1.254 / ws_x_scale, -0.1286 / ws_y_scale, 1.0857 / ws_z_scale])
    # via4 = np.array([4.26 / ws_x_scale, 4.76 / ws_y_scale, 3.56 / ws_z_scale])
