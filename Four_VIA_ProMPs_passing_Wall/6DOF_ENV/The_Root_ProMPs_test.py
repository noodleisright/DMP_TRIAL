...
class ProMP:
    ...
    def loadData(self, dt, training_address):
        ...
        demo = {"q": [], "qdot": [], "q_mean": [], "q_cov": [], "q_var": [], "qdot_mean": [], "qdot_cov": [],
                "qdot_var": []}
        for i in range(demo_data[0].shape[1] / 2):
            ...
            for j in range(len(demo_data)):
                ...
                q_dot.append(
                    demo_data[j][:, (2 * i) + 1].T)  # TODO: Velocity not recalculated like in MATLAB. VERIFY!!???
    ...
    def pmpRegression(self, data, nBasis=30):
        ...
        pmp = {"phase": phase, "w": weight, "basis": basis, "nBasis": nBasis, "nJoints": nJoints, "nDemo": nDemo,
               "nTraj": nTraj}
    ...
    def perJointPromp(self, xFull, Pfull, pmp):
        ...
        kf = {"w_mean": [], "w_sigma": [], "w_sigma_ii": [], "q_mean": [], "q_sigma_ii": [], "qdot_mean": [],
              "qdot_sigma_ii": []}
        for i in range(pmp["nJoints"]):
            ...
            q_mean, q_sigma_ii, qdot_mean, qdot_sigma_ii = self.thetaToTraj(kf["w_mean"][i], kf["w_sigma"][i],
